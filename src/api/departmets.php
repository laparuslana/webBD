...
if ($action == "read") {
    $result["departmets"] = "read";
    $stmt = $pdo->query("SELECT departaments.id,departaments.name,departaments.short_name, faculties.name as 'faculty' 
FROM departmets INNER JOIN faculty on departamets.faculty_id = faculty.id");
    $tmp = $pdo->query('SELECT COUNT(*) FROM departmets');
...
if ($action == "create") {
    ...
    $stmt_verify = $pdo->query("SELECT * FROM departmets WHERE name = '$name'");
    $find_stmt_verify_num_of_rows = $stmt_verify->rowCount();
    if ($find_stmt_verify_num_of_rows < 1) {
        $stmt = $pdo->query("INSERT INTO departmets (faculty_id, name, short_name) 
        VALUES ((SELECT f.id FROM faculty f WHERE f.short_name ='$faculty'), '$name','$short')");
        if ($stmt) {
...
if ($action == "delete") {
    ...
    $name = $_POST["name"];
    $short = $_POST["short"];
    $stmt_verify = $pdo->query("SELECT * FROM departmets WHERE id = '$id'");
    $find_stmt_verify_num_of_rows = $stmt_verify->rowCount();
    if ($find_stmt_verify_num_of_rows >= 1) {
        $stmt = $pdo->query("DELETE FROM departmets WHERE id = '$id'");
        if ($stmt) {
...
if ($action == "update") {
    ...
    $short = $_POST["short_name"];
    $faculty = $_POST["faculty"];
    $stmt_verify = $pdo->query("SELECT * FROM departmets WHERE name = '$name' and id != '$id'");
    $find_stmt_verify_num_of_rows = $stmt_verify->rowCount();
    if (!$find_stmt_verify_num_of_rows >= 1) {
        $stmt = $pdo->query("UPDATE departmets 
                                        SET
                                            faculty_id = (SELECT f.id FROM faculty f WHERE f.name ='$faculty'),
                                            name = '$name',
                                            short_name = '$short'
                                        WHERE
                                            id = '$id'
...
if ($action == "deleteAll") {
    $stmt = $pdo->query("DELETE FROM departmets");
    if ($stmt) {
