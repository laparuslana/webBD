...
if ($action == "create") {
    ...
    if ($find_stmt_verify_num_of_rows < 1) {
        ...
        VALUES ((SELECT d.id FROM departmets d WHERE d.short_name ='$departament'), '$name','$course')");
        if ($stmt) {
...
if ($action == "update") {
    ...
    if (!$find_stmt_verify_num_of_rows >= 1) {
        $stmt = $pdo->query("UPDATE groups 
                                        SET
                                            departament_id = (SELECT d.id FROM departmets d WHERE d.short_name ='$departament'),
                                            name = '$name',
                                            course = '$course'
                                        WHERE
                                            id = '$id'
